...
glm::mat4 EditorScene::LocalTransformComponent::calc_model_matrix() const {
     glm::mat4 model_matrix = glm::translate(position);
     model_matrix = glm::rotate(model_matrix, euler_rotation.x, glm::vec3(1.0f, 0.0f, 0.0f));
     model_matrix = glm::rotate(model_matrix, euler_rotation.y, glm::vec3(0.0f, 1.0f, 0.0f));
     model_matrix = glm::rotate(model_matrix, euler_rotation.z, glm::vec3(0.0f, 0.0f, 1.0f));
...
void EditorScene::LitMaterialComponent::add_material_imgui_edit_section(MasterRenderScene& /*render_scene*/, const SceneContext& /*scene_context*/) {
    ...
    ImGui::Text("Material");
    material_changed = ImGui::DragFloat("Texture Scale", &material.texture_scale, 0.01f);
    ...
    if (ImGui::Button("Reset Material")) {
        // Reset the material values to their initial state
        material.texture_scale = 1.0f;
        material.diffuse_tint = { 1.0f, 1.0f, 1.0f, 1.0f };
        material.specular_tint = { 1.0f, 1.0f, 1.0f, 1.0f };
        material.ambient_tint = { 1.0f, 1.0f, 1.0f, 1.0f };
        material.shininess = 128;
        material_changed = true; // Set the material as changed
    }
